...
import type { ListenerHandlerEvents } from "../../typings/events.ts";
 ...
 */
export default class ListenerHandler extends CitrusHandler {
	/**
	...
	public constructor(client: CitrusClient, options: CitrusHandlerOptions) {
		...
		if (!(classToHandle.prototype instanceof Listener || classToHandle === Listener)) {
			throw new CitrusError("INVALID_CLASS_TO_HANDLE", classToHandle.name, Listener.name);
		}
	...
	public addToEmitter(id: string): Listener {
		...
		if (!listener) throw new CitrusError("MODULE_NOT_FOUND", this.classToHandle.name, id);
		...
		if (!Util.isEventEmitter(emitter)) throw new CitrusError("INVALID_TYPE", "emitter", "EventEmitter", true);
	...
	public setEmitters(emitters: any): ListenerHandler {
		for (const [key, value] of Object.entries(emitters)) {
			if (!Util.isEventEmitter(value)) throw new CitrusError("INVALID_TYPE", key, "EventEmitter", true);
...
export default interface ListenerHandler extends CitrusHandler {
	/**
