...
import { ArgumentMatches } from "../../../util/Constants.ts";
import Command, { ArgumentGenerator } from "../Command.ts";
import type { ContentParserResult } from "../ContentParser.ts";
import Flag from "../Flag.ts";
import Argument, { ArgumentOptions } from "./Argument.ts";
...
export default class ArgumentRunner {
	...
	public runOne(message: Message, parsed: ContentParserResult, state: ArgumentRunnerState, arg: Argument): Promise<Flag | any> {
		...
		if (runFn == null) {
			throw new CitrusError("UNKNOWN_MATCH_TYPE", arg.match);
		}
